...
class App extends Component {
  ...
  onClickImag = imageUr => {
    ...
    if (imageUr === imageVisible) {
      const val = Math.ceil(Math.random() * imagesList.length - 1)
      this.setState(prevState => ({
        imageVisible: imagesList[val].imageUrl,
  ...
  render() {
    const {score, timeLeft, imageVisible, activeTabId, isTrue} = this.state
    const filteredList = this.getFilteredProjects()

    return (
      <div>
        <Navbar topScore={score} timeLeftSec={timeLeft} />

        <div className="bg-container">
          {!isTrue && (
            <>
              <div className="container">
                <img src={imageVisible} className="images" alt="match" />
              </div>
              <ul className="container-1">
                {tabsList.map(eachtab => (
                  <TabList
                    eachTab={eachtab}
                    onThumbnailClick1={this.onThumbnailClick}
              ...
              <ul className="thumbnail-image">
                {filteredList.map(eachImg => (
                  <ImagesList
                    eachImage={eachImg}
                    onClickImages={this.onClickImag}
                    key={eachImg.id}
                  />
                ))}
              </ul>
          ...
          {isTrue && (
            <div className="scoreboard">
              <div className="score-container">
                ...
                />
                <div>
                  <p className="scoreboard-para">Your Score</p>
